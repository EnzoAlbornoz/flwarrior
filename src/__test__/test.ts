...
test("Test Grammar creation", () => {
    ...
    expect(Array.from(gram.nonTerminalSymbols.symbols).toString()).not.toContain(new AlphabetSymbol('X').toString());
    expect(Array.from(gram.nonTerminalSymbols.symbols).toString()).toContain(new AlphabetSymbol('S').toString());
    expect(Array.from(gram.nonTerminalSymbols.symbols).toString()).toContain(new AlphabetSymbol('A').toString());
    expect(Array.from(gram.nonTerminalSymbols.symbols).toString()).toContain(new AlphabetSymbol('B').toString());
    
    expect(Array.from(gram.terminalSymbols.symbols).toString()).not.toContain(new AlphabetSymbol('X').toString());
    expect(Array.from(gram.terminalSymbols.symbols).toString()).toContain(new AlphabetSymbol('a').toString());
    expect(Array.from(gram.terminalSymbols.symbols).toString()).toContain(new AlphabetSymbol('b').toString());
    expect(Array.from(gram.terminalSymbols.symbols).toString()).toContain(AlphabetSymbol.EPSILON.toString());
...
test("Test Finite State Machine creation", () => {
    var machine = buildRegularSadFiniteMachine();
    ...
    expect(Array.from(machine.alphabet.symbols).map((alphabetSymbol: AlphabetSymbol) => alphabetSymbol.toString())).toEqual(expect.arrayContaining(['b','a','ε']));
...
function buildRegularSadFiniteMachine(): FiniteStateMachine {
    var entryState = new State("q0", true, true);
    var otherState = new State("q1", false, true);
    return new FiniteStateMachine(
                            "sad",
                            "id1",
                            new Set([entryState, otherState]),
                            new Alphabet(new Set([new AlphabetSymbol("b"), new AlphabetSymbol("a"), AlphabetSymbol.EPSILON])),
                            new Array([[entryState, new AlphabetSymbol("a")], entryState],
